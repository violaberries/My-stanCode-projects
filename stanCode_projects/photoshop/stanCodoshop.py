...
def get_pixel_dist(pixel, red, green, blue):
    ...
    """
    color_d = math.sqrt((red - pixel.red)**2 + (green - pixel.green)**2 + (blue - pixel.blue)**2)
    return color_d
...
def get_best_pixel(pixels):
    ...
    lst = []
    # create a dict => key: value => color_d: pixel
    d = {}
    for pixel in pixels: # find color distance of each pixel
        color_d = get_pixel_dist(pixel, red, green, blue)
        lst.append(color_d)
        d[color_d] = pixel
    min_dis = min(lst)
    pixel = d[min_dis]
...
def solve(images):
    ...
    for x in range(width):
        for y in range(height): # find (x, y)
            pixel_list = []  # list of pixel(x, y) in each picture
            r_pixel = result.get_pixel(x, y)

            for pic in images:  # loop over the images, look at each picture
                # pic's data type: image
                pic_pixel = pic.get_pixel(x, y)
                pixel_list.append(pic_pixel)

            best = get_best_pixel(pixel_list)
            r_pixel.red = best.red
            r_pixel.green = best.green
            r_pixel.blue = best.blue
